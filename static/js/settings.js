...
function initSettingsModal() {
  ...
  document.querySelectorAll(".admin-subtab").forEach((tab) => {
    tab.addEventListener("click", () => {
      ...
      // Update active tab
      document
        .querySelectorAll(".admin-subtab")
        .forEach((t) => t.classList.remove("active"));
      tab.classList.add("active");

      // Show target panel
      document
        .querySelectorAll(".admin-subpanel")
        .forEach((p) => p.classList.remove("active"));
...
function updateSettingsUI() {
  ...
  // Radio buttons
  document.querySelector(
    `input[name="iconSize"][value="${settings.iconSize}"]`,
  ).checked = true;
  document.querySelector(
    `input[name="gridDensity"][value="${settings.gridDensity}"]`,
  ).checked = true;
  document.querySelector(
    `input[name="autoRefresh"][value="${settings.autoRefresh}"]`,
  ).checked = true;
...
function updateSettingsUserSection() {
  ...
  if (isAuthenticated && currentUser) {
    ...
    document.getElementById("settingsUserAvatar").textContent =
      currentUser.username[0].toUpperCase();
    document.getElementById("settingsUserName").textContent =
      currentUser.displayName || currentUser.username;
    document.getElementById("settingsUserSource").textContent =
    ...
    if (prefsSyncEnabled) {
      ...
      syncStatus.innerHTML =
        '<span class="settings-sync-dot"></span><span>Synced</span>';
    } else {
      syncStatus.classList.add("offline");
      syncStatus.innerHTML =
        '<span class="settings-sync-dot"></span><span>Local only</span>';
    }
  } else {
...
async function loadProfile() {
  ...
  // Show/hide password section based on auth source
  const passwordSection = document.getElementById("profilePasswordSection");
  if (currentUser && currentUser.source === "local") {
...
function toggleCSSSnippet(index) {
  ...
  renderCSSSnippets();
  showToast(
    css.includes(firstLine)
      ? `${snippet.name} enabled`
      : `${snippet.name} removed`,
  );
}
