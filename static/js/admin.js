...
async function loadAdminData() {
  ...
  try {
    ...
    if (adminState.localAuthEnabled) {
      const localUsersResp = await fetch("/api/admin/local-users", {
        credentials: "include",
      });
    ...
    adminState.groups = Array.from(allGroupNames)
      .sort()
...
async function reloadLocalUsers() {
  if (!adminState.isAdmin) return;
  try {
    const resp = await fetch("/api/admin/local-users", {
      credentials: "include",
    });
    if (resp.ok) {
...
async function loadSystemConfig() {
  try {
    ...
    if (resp.ok) {
      ...
      // General settings
      document.getElementById("systemSessionDays").value =
        config.sessionDays || 7;
      document.getElementById("systemCookieSecure").checked =
        config.cookieSecure !== false;

      // Security settings
      document.getElementById("systemAdminGroup").value =
        config.adminGroup || "admin";

      // Auth providers
      document.getElementById("systemProxyAuth").checked =
        config.proxyAuthEnabled || false;
      document.getElementById("systemTrustedProxies").value =
        config.trustedProxies || "";
      document.getElementById("systemLocalAuth").checked =
        config.localAuthEnabled || false;
      document.getElementById("systemLDAPAuth").checked =
        config.ldapAuthEnabled || false;
      document.getElementById("systemOIDCAuth").checked =
        config.oidcAuthEnabled || false;
      document.getElementById("systemAPIKeys").checked =
      ...
      document.getElementById("ldapUserFilter").value =
        config.ldapUserFilter || "(uid=%s)";
      document.getElementById("ldapGroupFilter").value =
        config.ldapGroupFilter || "(memberUid=%s)";
      document.getElementById("ldapUserAttr").value =
        config.ldapUserAttr || "uid";
      document.getElementById("ldapEmailAttr").value =
        config.ldapEmailAttr || "mail";
      document.getElementById("ldapDisplayAttr").value =
        config.ldapDisplayAttr || "cn";
      document.getElementById("ldapGroupAttr").value =
        config.ldapGroupAttr || "memberOf";
      document.getElementById("ldapStartTLS").checked =
        config.ldapStartTLS || false;
      document.getElementById("ldapSkipVerify").checked =
        config.ldapSkipVerify || false;

      // OIDC settings
      document.getElementById("oidcDisplayName").value =
        config.oidcDisplayName || "";
      document.getElementById("oidcIssuer").value = config.oidcIssuer || "";
      document.getElementById("oidcClientID").value = config.oidcClientID || "";
      document.getElementById("oidcRedirectURL").value =
        config.oidcRedirectURL || "";
      document.getElementById("oidcScopes").value =
        config.oidcScopes || "openid profile email groups";
      document.getElementById("oidcGroupsClaim").value =
...
function toggleTrustedProxiesSection() {
  ...
  document.getElementById("trustedProxiesSection").style.display = enabled
    ? "block"
    : "none";
  updateProxyAuthWarning();
}
...
function toggleLDAPSection() {
  ...
  document.getElementById("ldapConfigSection").style.display = enabled
    ? "block"
    : "none";
}
...
function toggleOIDCSection() {
  ...
  document.getElementById("oidcConfigSection").style.display = enabled
    ? "block"
    : "none";
}
...
function toggleAPIKeySection() {
  ...
  document.getElementById("apiKeysSection").style.display = enabled
    ? "block"
    : "none";
...
function toggleLocalAuthSection() {
  ...
  document.getElementById("localUsersSection").style.display = enabled
    ? "block"
    : "none";
  document.getElementById("localUsersDivider").style.display = enabled
    ? "block"
    : "none";
}
...
async function saveSystemSettings() {
  ...
  if (oidcAuthEnabled) {
    ...
    const oidcRedirectURL = document
      .getElementById("oidcRedirectURL")
      .value.trim();
  ...
  const payload = {
    sessionDays:
      parseInt(document.getElementById("systemSessionDays").value) || 7,
    cookieSecure: document.getElementById("systemCookieSecure").checked,
    adminGroup:
      document.getElementById("systemAdminGroup").value.trim() || "admin",
    proxyAuthEnabled,
    trustedProxies: document
      .getElementById("systemTrustedProxies")
      .value.trim(),
    localAuthEnabled,
    ldapAuthEnabled,
    oidcAuthEnabled,
    ...
    ldapUserFilter:
      document.getElementById("ldapUserFilter").value.trim() || "(uid=%s)",
    ldapGroupFilter:
      document.getElementById("ldapGroupFilter").value.trim() ||
      "(memberUid=%s)",
    ldapUserAttr: document.getElementById("ldapUserAttr").value.trim() || "uid",
    ldapEmailAttr:
      document.getElementById("ldapEmailAttr").value.trim() || "mail",
    ldapDisplayAttr:
      document.getElementById("ldapDisplayAttr").value.trim() || "cn",
    ldapGroupAttr:
      document.getElementById("ldapGroupAttr").value.trim() || "memberOf",
    ldapStartTLS: document.getElementById("ldapStartTLS").checked,
    ldapSkipVerify: document.getElementById("ldapSkipVerify").checked,
    // OIDC settings
    oidcDisplayName: document.getElementById("oidcDisplayName").value.trim(),
    ...
    oidcScopes:
      document.getElementById("oidcScopes").value.trim() ||
      "openid profile email groups",
    oidcGroupsClaim:
      document.getElementById("oidcGroupsClaim").value.trim() || "groups",
  };
...
function renderAPIKeysList() {
  ...
  if (!adminState.apiKeys || adminState.apiKeys.length === 0) {
    container.innerHTML =
      '<div class="admin-empty">No API keys. Click "Create Key" to add one.</div>';
    return;
  }

  container.innerHTML = adminState.apiKeys
    .map(
      (key) => `
...
async function restoreFromBackup(input) {
  ...
  try {
    ...
    const result = await resp.json();
    showToast(
      `Restored: ${result.restored.systemConfig} config, ${result.restored.userPreferences} preferences`,
    );
